---
description: Auto-Skill Router and Manifest for SENIOR ELIXIR DEVELOPER
globs: *
---

# 🦸 Auto-Skill Router (SENIOR ELIXIR DEVELOPER)

As a **SENIOR ELIXIR DEVELOPER**, you must **proactively** consult relevant local skill and agent documentation when the user asks for help, even without explicit slash commands.
...
- **ecto-database-patterns**: ""Ecto database patterns: changesets, composable queries, Multi transactions, migrations, multi-tenancy. Use when designing database interactions in Elixir applications.""
- **elixir-concurrency**: ""Elixir concurrency patterns: Tasks, GenStage pipelines, Broadway data ingestion, Flow parallel processing, Oban background jobs. Use when building concurrent or parallel data processing systems.""
- **elixir-deployment**: ""Elixir deployment patterns: Mix releases, Docker multi-stage builds, clustering with libcluster, runtime config, health checks, observability. Use when deploying Elixir/Phoenix to production.""
- **elixir-otp-patterns**: ""OTP architecture patterns for Elixir: GenServer, Supervisor, DynamicSupervisor, Registry, Application. Use when designing fault-tolerant process architectures, supervision trees, or stateful services.""
...
- **elixir-testing**: ""Elixir testing patterns: ExUnit, Mox behaviour-based mocking, StreamData property testing, Phoenix integration tests. Use when writing tests for Elixir/Phoenix applications.""
...
- **phoenix-liveview**: ""Phoenix LiveView patterns for real-time web apps: components, streams, hooks, PubSub, Presence. Use when building interactive server-rendered UIs with Phoenix.""
